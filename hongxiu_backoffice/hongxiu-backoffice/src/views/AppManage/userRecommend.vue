<template>
  <div>
    <div class="backGround">
      ...
      <el-row>
        <div>
          <el-button type="primary" icon="el-icon-plus" @click="addUserDialog = true">新增</el-button>
      ...
      <el-row>
        <el-table
          ...
          <el-table-column
              prop="operatingInstructions"
              label="操作原因"
              align="center"
              header-align="center"
          >
            <template slot-scope="scope">
              <el-link type="primary"  @click="openRefuseReason(scope.row.txt)">查看</el-link>
            </template>
          </el-table-column>
          <el-table-column
              prop="createId"
              ...
              header-align="center">
            <template slot-scope="scope">
              <span v-if="!scope.row.createId">-</span>
          ...
          <el-table-column
              ...
              header-align="center">
            <template slot-scope="scope">
              <el-link type="danger" @click="deletedUser(scope.row.id)">删除</el-link>
            </template>
          </el-table-column>
        </el-table>
    ...
    >
      <el-form ref="form" label-width="80px">
        <el-form-item label="用户ID">
          <el-input placeholder="请输入内容" v-model="addUserId" class="input-with-select">
            <el-button slot="append" icon="el-icon-search" @click="getUser"></el-button>
          </el-input>
        </el-form-item>
        <el-form-item label="用户名">
          <span v-if="userDetail===''">--</span>
          <span v-if="userDetail!==''">{{ userDetail.nickname}}</span>
        </el-form-item>
        <el-form-item label="简介">
          <span v-if="userDetail===''">--</span>
          <span v-if="userDetail!==''">{{ userDetail.signatureText}}</span>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
       <el-button @click="addUserDialog = false;userDetail = '';addUserId='';reason=''">取 消</el-button>
        <el-button type="primary" @click="addUserRecommend">确 定</el-button>
      </span>
    </el-dialog>
  ...
  methods:{
    ...
    deletedUser(id){
      this.$confirm('确定删除这条推荐?', '提示', {
        ...
        this.$axios.post(this.$getUrl("用户推荐删除"),{ids:[id]},{
          transformRequest:[
            function(data){
              let params = "";
              var arr = [];
    ...
    addUserRecommend(){
      this.$axios.post(this.$getUrl("用户推荐新增"),{userId:this.addUserId,log:this.reason},{
        transformRequest:[
          function(data){
            let params = "";
            var arr = [];
    ...
    getUser(){
      this.$axios.post(this.$getUrl("用户详情"),{userId:this.addUserId},{
        transformRequest:[
          function(data){
            let params = "";
            var arr = [];
    ...
    getUserList(){
      ...
      this.$axios.post(this.$getUrl("用户推荐列表"),data,{
        transformRequest:[
          function(data){
            let params = "";
            var arr = [];
